...
```math
\begin{aligned}
D^2 u(\bar{x}) & =\hat{D}_0\left(\hat{D}_0 u(\bar{x})\right) \\ & =\frac{1}{h}\left(\left(\frac{u(\bar{x}+h)-u(\bar{x})}{h}\right)-\left(\frac{u(\bar{x})-u(\bar{x}-h)}{h}\right)\right) \\ & =\frac{1}{h^2}(u(\bar{x}-h)-2 u(\bar{x})+u(\bar{x}+h)) \\ & =u^{\prime \prime}(\bar{x})+\frac{1}{12} h^2 u^{(4)}(\bar{x})+O\left(h^4\right)
\end{aligned}
```
In fact, 
